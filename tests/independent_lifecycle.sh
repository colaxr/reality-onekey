...
write_socks_env 192.0.2.1 31080 test-user test-pass Test 192.0.2.1 hev
apply_socks_change "$TEST_ROOT/new-backup" >/dev/null
[[ "$X_RESTARTS" == 0 ]]
cmp "$CONFIG_FILE" "$TEST_ROOT/ss-original.json"
[[ "$(managed_listeners)" != *31080* ]]
grep -q 'udp-port: 31080' "$SOCKS_CONFIG"
mkdir "$TEST_ROOT/edit-backup"
backup_state "$TEST_ROOT/edit-backup"
cp "$SOCKS_CONFIG" "$TEST_ROOT/edit-backup.yml"
write_socks_env 192.0.2.1 31081 other-user other-pass Other 192.0.2.1 hev
START_OK=false
...
cmp "$SOCKS_CONFIG" "$TEST_ROOT/edit-backup.yml"
START_OK=true
